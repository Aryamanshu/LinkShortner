...
export default function GitHubAuth() {
  ...
  useEffect(() => {
    ...
    const handleGitHubAuth = async () => {
      try {
        ...
        const response = await fetch(`${URI}/api/auth/github`, {
          method: 'POST',
          headers: {
        ...
        if (response.ok) {
          ...
          toast.error(result.error || 'Failed to authenticate with GitHub');
        }
      } catch (error) {
        console.error('GitHub auth error:', error);
        setError('An error occurred during GitHub authentication');
        toast.error('An error occurred during GitHub authentication');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <Layout title="GitHub Authentication - TinyHost" showHeader={false} showFooter={false}>
        ...
        <div className="max-w-md w-full p-8 bg-dark-800 rounded-xl shadow-xl border border-teal-700/30">
          <div className="text-center">
            <h1 className="text-2xl font-bold text-beige-500 mb-4">
              {isLoading ? 'Authenticating with GitHub...' : error ? 'Authentication Failed' : 'Authentication Successful'}
            </h1>
            ...
            {isLoading ? (
              ...
              <div className="text-center my-8">
                ...
                <p className="text-beige-600">You have successfully signed in with GitHub!</p>
